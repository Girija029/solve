...
app = Flask(__name__)
app.secret_key = 'kce_erp_secret_key_2024_xYzAbC'
...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO settings (id, academic_year, semester) VALUES (1, '2024-2025', 'Even')")
    ...
    try:
        c.execute("ALTER TABLE subjects ADD COLUMN subject_code TEXT DEFAULT ''")
    except:
        pass
    try:
        c.execute("ALTER TABLE subjects ADD COLUMN staff_name TEXT DEFAULT ''")
    except:
        pass
...
def login():
    ...
    if request.method == 'POST':
        ...
        if username == 'user' and password == 'user123':
            ...
            error = "Invalid Credentials! Use user / user123"
...
def admin():
    ...
    if request.method == 'POST':
        ...
        if action == 'add_class':
            ...
            if subject_name and class_name:
                c.execute("INSERT INTO subjects (name, class_name, subject_code, staff_name) VALUES (?, ?, ?, ?)", 
                         (subject_name, class_name, subject_code, staff_name))
    ...
    exams = c.fetchall()
    c.execute("SELECT id, name, class_name, subject_code, staff_name FROM subjects")
    subjects = c.fetchall()
...
def report():
    ...
    c.execute("SELECT name, subject_code, staff_name FROM subjects WHERE class_name=?", (class_name,))
    subject_rows = c.fetchall()
    active_subjects = [row[0] for row in subject_rows]
    subject_info = {row[0]: {'code': row[1] or '', 'staff': row[2] or ''} for row in subject_rows}
    ...
    total_appeared = sum(subject_counts.values())
    total_passed = sum(subject_pass.values())
    overall_pass_pct = round((total_passed / total_appeared * 100), 2) if total_appeared > 0 else 0
